...
var mercatorTw = () => {
  ...
  const mainland = geoMercator().rotate([-120.97, -23.6]);
  let mainlandPoint;
  const penghu = geoMercator().rotate([-119.53, -23.47]);
  let penghuPoint;
  const kinmen = geoMercator().rotate([-118.38, -24.44]);
  let kinmenPoint;
  const lienchiang = geoMercator().rotate([-120.22, -26.16]);
  let lienchiangPoint;
  const wuqiu = geoMercator().rotate([-119.45, -24.98]);
  let wuqiuPoint;
  ...
  mercatorTw.fitExtent = (extent, object) =>
    fitExtent(mercatorTw, extent, object);
